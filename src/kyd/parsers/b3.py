...
class TaxaSwap:
    ...
    def __init__(self, rawdata):
        self.__data = read_fwf(rawdata, self.widths, self.colnames,
                               parse_fun=self._parse)
        self.__findata = [self._build_findata(list(v))
                          for k, v in groupby(self.__data,
                                              key=lambda x: x['cod_taxa'])]

    def _parse(self, obj):
        obj['data_geracao_arquivo'] = '{}-{}-{}' \
            .format(obj['data_geracao_arquivo'][:4],
                    obj['data_geracao_arquivo'][4:6],
                    obj['data_geracao_arquivo'][6:])
    ...
    def _build_findata(self, lst):
        ...
        terms = [dict(zip(keys, x)) for x in zip(num_dias_corridos,
                                                 num_dias_saques,
                                                 carat_vertice,
                                                 taxa_teorica)]
...
def parse_taxa_cdi_idi_cetip(fname, text):
    rex_date = re.compile(r'^.*(\d{8}).*$')
    m = rex_date.match(fname)
    date = m.group(1)
    date = '{}-{}-{}'.format(date[:4], date[4:6], date[6:8])
    return int(text)/100, date
