...
def handle_informes_diarios(row):
    ...
    fields = [parser.parse(val.strip()) for val in row.split(';')]
    fields = [None if val == '' else val for val in fields]
    row = dict(zip(names, fields))
    cnpj_fundo_str_num = row['cnpj_fundo'] \
        .replace('.', '') \
        .replace('/', '') \
        .replace('-', '')
...
def handle_info_cadastral(row):
    ...
    names = ['cnpj_fundo', 'denom_social', 'dt_reg', 'dt_const', 'dt_cancel',
             'sit', 'dt_ini_sit', 'dt_ini_ativ', 'dt_ini_exerc',
             'dt_fim_exerc', 'classe', 'dt_ini_classe', 'rentab_fundo',
             'condom', 'fundo_cotas', 'fundo_exclusivo', 'trib_lprazo',
             'invest_qualif', 'taxa_perfm', 'inf_taxa_perfm', 'taxa_adm',
             'inf_taxa_adm', 'vl_patrim_liq', 'dt_patrim_liq', 'diretor',
             'cnpj_admin', 'admin', 'pf_pj_gestor', 'cpf_cnpj_gestor',
             'gestor', 'cnpj_auditor', 'auditor', 'cnpj_custodiante',
